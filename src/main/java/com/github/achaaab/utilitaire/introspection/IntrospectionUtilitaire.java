...
public class IntrospectionUtilitaire {
	 ...
	 */
	private static final String MESSAGE_APPEL_INTERDIT = "L''appel de la méthode {0} "
			+ "sur un objet de type {1} est interdit, "
			+ "vérifier que la méthode en question est publique.";
	...
	private static final String MESSAGE_MAUVAIS_ARGUMENTS = "Le type des arguments founis ({0}) "
			+ "ne correspond pas à la signature de la méthode appelée ({1}) du type {2}.";
	 ...
	 */
	private static final Map<Class<?>, Class<?>> tableWrappers;

	static {

		tableWrappers = new HashMap<>();
	...
	/**
	 * @param classe classe dont on souhaite creer une instance
	 * @param typesParametres types des parametres du constructeur a appeler
	 * @param valeursParametres valeurs des parametres du constructeur a appeler
	...
	public static Object creerInstance(Class<?> classe,
		...
		String detailConstructeur = getNomMethode(nomConstructeur, typesParametres);
		String detailParametres = getParametresEffectifs(valeursParametres);
		Object[] parametresMessage = {nomClasse, detailConstructeur, detailParametres};
		String message = MessageFormat.format(MESSAGE_ERREUR_INSTANCIATION, parametresMessage);

		try {

			Constructor<?> constructeur = classe.getConstructor(typesParametres);
	 ...
	 *
	 * @param bean le bean dont on souhaite obtenir la valeur d'une propriete
	 * @param propriete la propriete du bean dont on souhaite obtenir la valeur
	 * (exemple : nom, prenom, personne.nom, personne.pere.age, ...)
	 * @return la valeur de la propriete donnee dans le bean donne
	 * @throws ExceptionIntrospection exception lors de l'introspection
	 */
	 ...
	 *
	 * @param bean le bean dans lequel on souhaite ins�rer la valeur d'une
	 * propriete
	 * @param propriete la propriete du bean que l'on souhairte valoris� (exemple :
	 * nom, prenom, personne.nom, personne.pere.age, ...)
	 * @param valeur valeur � ins�rer dans le champ du bean (pour les types
	 * primitifs, utiliser le wrapper)
	 * @param primitif bool�en indiquant si l'objet pass� en param�tre est de type
	 * primitif
	 * @throws ExceptionIntrospection exception lors de l'introspection
	 */
	 ...
	 *
	 * @param bean bean dont on souhaite obtenir la valeur d'un champ
	 * @param nomChamp nom du champ du bean dont on souhaite obtenir la valeur
	 ...
	 *
	 * @param bean bean dans lequel on souhaite modifier la valeur d'un champ
	 * @param nomChamp nom du champ du bean dont on souhaite modifier la valeur
	 * @param valeur valeur à insérer dans le champ du bean (pour les types
	 * primitifs, utiliser le wrapper)
	 * @param primitif booléen indiquant si l'objet passé en paramètre est de type primitif
	 * @throws ExceptionIntrospection exception lors de l'introspection
	 */
	...
	/**
	 * @param objet objet sur lequel appeler la methode (null pour appel d'une
	 * methode statique)
	 * @param nomMethode nom de la methode
	 * @param typesParametres tableau contenant le type des parametres, attention si le
	 * parametre effectif est de type primitif il faut utiliser
	 * l'attribut TYPE du wrapper correspondant
	 * @param parametres tableau des parametres effectifs eventuellement encapsules
	...
	/**
	 * @param nomMethode nom de la methode sans parentheses ni arguments
	 ...
	 *
	 * @param classe une classe quelconque
